...
- ~~약점: 신뢰할 UI 입력 수단 없음~~ → **해결됨.** `tap`/`swipe`/`text` 를 VM Service로 구동
  ...
  입력 백엔드를 VM Service로 통일(2026-07-18). UI **단언**("이 버튼이 보이나?")은 여전히 emu 영역 밖.</new_string>
</invoke>
...
완료: #1·#2·#3·#5·#7·#8·#9·#10, #6은 `inspect` 슬라이스 구현
...
(예: 웹뷰 폼 입력 요구 → #11의 idb 경로, probe 한계 신호 → #6).
