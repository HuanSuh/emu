...
| 11 | iOS 텍스트 입력 IME 우회(현재 포커스된 Flutter 필드 한정) | 낮음 | M | P3 | ⬜ |
...
- ⬜ swipe 를 VM Service 로 옮기면 `delta`·`pointer` id·`timeStamp` 필수(위 iOS swipe 참고).
...
- ⬜ **남은 것**: iOS 입력 수단 → **조사 + iOS 실기 검증 완료**(위). VM Service 합성 포인터로 tap·swipe
  모두 실측됨. **이제 구현 단계**: `tap`/`swipe` 에 vm-service 백엔드를 추가하고 플랫폼으로 자동 선택,
  사용자에겐 물리 픽셀 좌표 하나로 통일(백엔드가 dpr로 나눔). `text` 는 별도(TextInput 채널).
...
보류: #4(e2e — 전제 소멸), #6(Tier2 디버거 — probe와 중복).

**남은 것 (우선순위 낮음)**
- #11 iOS 텍스트 입력 IME 우회(P3): 현재 경로는 포커스된 Flutter 필드 한정.
- Tier2 디버거(#6)는 실사용에서 probe로 부족하다는 신호가 쌓이면 재검토.
