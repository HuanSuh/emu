...
설계 불변식(변경이 넘어선 안 되는 선)은 [`DESIGN.md`](DESIGN.md) 참고.
...
- ~~약점: 신뢰할 UI 입력 수단 없음~~ → **해결됨.** `tap`/`swipe`/`text` 를 VM Service로 구동
  (Android·iOS, 유니코드). "adb tap이 안 먹는다"는 초기 단정은 반증됐고(2026-07-17), 이후
  입력 백엔드를 VM Service로 통일(2026-07-18). UI **단언**("이 버튼이 보이나?")은 여전히 emu 영역 밖.
...
| 5 | `emu tap`/`swipe`/`text` — 입력 추상화 (VM Service, Android·iOS, 유니코드) | **높음** | **S** | **P1** | ✅ 완료 |
| 6 | Tier 2 디버거 — `inspect`(지역변수+콜스택 스냅샷) ✅ / 대화형 break·step 보류 | 중~높음 | M+L | P2 | 부분 완료 |
| 7 | iOS 시뮬레이터 실기 검증 (기동 + 입력) | 중 | S | P2 | ✅ 완료 |
| 8 | 대시보드 — 인터랙티브 스크린샷(클릭=탭/드래그=스와이프/텍스트) + DevTools 링크 수정 | 낮음 | S | P3 | ✅ 완료 |
| 9 | `bootAndroid` AVD 선택 개선 + cold-boot 대기(`up --timeout`) | 중 | S | P2 | ✅ 완료 |
| 10 | 기동 인체공학 — `-d` iOS 자동 부팅, 실패한 `up` 정리, `reload` restart 힌트 | 중 | S | P2 | ✅ 완료 |
...
**iOS 실기 검증 완료 (2026-07-18)** — Xcode에 iOS 26.5 플랫폼 설치 후 재검증.
실제 iOS 26.5 시뮬레이터(iPhone 17)에서 VM Service 합성 포인터가 동작함을 확정:
- ✅ **tap** — 프레임워크 라이브러리 대상 `evaluate` 로 down/up 캐스케이드 dispatch →
  `gesture onTap count=1` 확인. **논리 좌표**(물리 601,711 ÷ dpr 3.0 = 200,237) 사용.
  simctl에 입력 명령이 없는데도 adb 없이 탭이 먹었다. Android 프록시 검증이 iOS 실기에서 확정됨.
- ✅ **swipe/scroll** — `PointerMoveEvent` 에 **`delta`·일관된 `pointer` id·`timeStamp`가 필수**다.
  position만 주면 스크롤 인식기가 드래그를 감지 못 한다(첫 시도 실패 → delta 추가 후 성공).
  timeStamp 기반 velocity로 fling이 걸리므로(offset이 크게 튐) 세기 조절엔 스텝 수/간격 튜닝 필요.
- ✅ `emu shot` 은 iOS에서 simctl 경로로 이미 정상 동작(물리 픽셀).

**미해결 / 다음 단계**
- ✅ **기동 버그 수정됨(2026-07-18)**: `-d <udid>` 가 iOS 시뮬레이터 udid면 `_launch` 가
  `bootIos(udid:)` 를 먼저 태운다(idempotent — 이미 부팅됐으면 no-op). 실측: 시뮬레이터를 완전히
  끈 상태에서 `emu up -d <udid>` → 자동 부팅 후 `running`/`firstFrame:true`. `booting iOS
  simulator: <udid>` 로그로 경로 확인. `platformForDeviceId` 로 판별하므로 Android `-d` 는 영향 없음.

### text 입력 — VM Service `updateEditingValue` 가 통합 해법 (2026-07-18)
"iOS text는 이 경로로 안 풀린다"던 추정을 **뒤집었다.** 포인터와 똑같은 프레임워크 레벨 경로로 풀린다.

- ✅ **iOS·Android 양쪽에서 실측 성공.** 포커스된 `EditableTextState` 를 찾아 `updateEditingValue`
  를 직접 호출 = 엔진이 사용자 입력 시 부르는 바로 그 메서드. adb·simctl·플랫폼 키보드 전부 불필요.
  ```
  target lib: package:flutter/src/widgets/editable_text.dart
  expr: (FocusManager.instance.primaryFocus!.context!
          .findAncestorStateOfType<EditableTextState>()!
          ..updateEditingValue(TextEditingValue(text: "..."))).runtimeType.toString()
  ```
  (필드를 먼저 합성 포인터로 tap 해 포커스시킬 것.)
- 🎯 **유니코드가 된다 — 양 플랫폼 모두.** `한글 유니코드 😀` 를 iOS·Android 둘 다 정확히 주입.
  이는 **adb `input text` 의 ASCII 한계를 근본 우회**한다(adb 경로는 비ASCII에서 NPE로 죽었다).
- 📌 **`evaluate` 함정**: `updateEditingValue` 는 `void` 반환 → `.toString()` 붙이면 컴파일 에러.
  캐스케이드(`..`)로 감싸 대상을 되돌려야 한다(`(x..updateEditingValue(...)).runtimeType`).

**구현 완료 (2026-07-18)** — `lib/src/input.dart` 신규. tap/swipe/text를 VM Service로 통일.
- ✅ Android·iOS 실측(프로덕션 명령): tap→gesture, swipe→scroll, text 유니코드(`한글 hello 😀`),
  `--append` 모두 통과. dpr 나눗셈은 `_dpr` 표현식으로 on-device 처리 → 사용자는 물리 픽셀만.
- ✅ adb 기반 `tap`/`swipe`/`typeText` 및 escape 헬퍼(`escapeAdbText`/`nonTypableChars`) 제거.
  스크린샷은 여전히 adb/simctl(`_serial`/`_platform` 유지). 코드 경로가 하나로 줄었다.
- ✅ `text --append` — 현재 값을 읽어 이어붙임. 포커스 필드 없으면 `no focused text field` 에러.
- 📌 `updateEditingValue` 의 selection offset은 UTF-16 길이(`String.length`) — 이모지는 surrogate로
  2를 차지하므로 host의 `.length` 가 on-device와 일치한다(별도 계산 불필요).

**설계 재정리 — 입력 백엔드 통합** *(구현으로 반영됨, 참고용 보존)*
- `tap`/`swipe`/`text` 를 **VM Service 백엔드로 통일** 가능해졌다(포인터+텍스트 전부 프레임워크 레벨).
  플랫폼 무관 + iOS 지원 + 유니코드 텍스트가 한 번에 풀린다.
- 트레이드오프: **Flutter 위젯 한정**(네이티브 뷰/웹뷰의 텍스트 필드엔 안 먹음 — emu는 Flutter 전용이라 무방).
  값을 **통째로 교체**하므로 append 하려면 현재 값을 읽어 이어붙여야 한다(제어 가능).
  IME 컴포징을 거치지 않고 **최종 텍스트를 결정적으로** 넣는다(자동화엔 오히려 장점).
- 갈래: (a) adb/simctl 스크린샷·부팅은 유지하되 **입력만 VM Service 로**, 또는 (b) Android는 adb 유지 +
  iOS만 VM Service. **(a) 추천** — 유니코드까지 얻고 코드 경로가 하나로 준다.
- ✅ swipe 를 VM Service 로 옮김 — `delta`·`pointer` id·`timeStamp` 반영(`input.dart`, 구현 완료).
...
- ✅ **iOS 입력 수단 구현 완료**(`input.dart`): `tap`/`swipe`/`text` 를 VM Service로 통일,
  사용자에겐 물리 픽셀 좌표 하나로 노출(백엔드가 dpr로 나눔). `text` 는 포커스된 EditableTextState에
  `updateEditingValue` — Flutter 위젯 한정(네이티브/웹뷰 필드는 #11로 남김).
...
### 6. Tier 2 인터랙티브 디버거 → **가치 있는 슬라이스만 구현(2026-07-18)**
- ✅ **`emu inspect <file:line>` 구현**: 브레이크포인트 히트 시 프레임의 **모든 지역변수 + 콜스택**을
  덤프하고 자동 resume. probe와 같은 단발성·비차단 모델이라 emu에 맞고, probe보다 풍부하다
  (변수명 미지정). `runInspect`(probe 인프라 재사용) + `/api/inspect` + CLI. 실기 검증:
  `inspect lib/main.dart:34` → `before=1 next=2 label="gesture #2" this=<_DemoPageState>` + 콜스택.
  📌 클로저 줄(`setState(() => …)`)은 최상위 프레임에 로컬이 없다 → 메서드 본문 줄을 inspect(문서화).
- 🚫 **대화형 `break`/`step`/`continue` 는 보류**: 명령 간 **앱을 멈춰둬야** 하는데, 그러면 로그·probe가
  멎고 emu의 무상태 단발성 모델과 충돌한다(e2e 동시검증과도 양립 불가). 에이전트는 대화형 REPL이
  아니라 단발 명령으로 일하므로 적합성도 낮다. **`inspect`/`probe` 로 부족하다는 실수요가 생기면 재검토.**
- **노력**: `inspect` 슬라이스 M(완료). 완전 대화형 디버거 L(보류).
...
### 8. 대시보드 범위 결정 → **차별화로 결론(2026-07-18)**
- 판단: 로그 뷰어는 DevTools+IDE에 열위라 다듬지 않는다. 대신 **그들이 못 하는 것 — 좌표로 앱을
  직접 모는 것** 에 투자한다(emu의 정체성인 입력 백엔드를 사람이 쓰게).
- ✅ **인터랙티브 스크린샷 패널 구현**: 기기 화면 표시 + 클릭=탭 / 드래그=스와이프 / 텍스트 입력(유니코드).
  `/api/screenshot?inline=1`(PNG 바이트 직접 반환) + 브라우저에서 물리 픽셀로 좌표 매핑.
  실제 브라우저(claude-in-chrome)로 tap/swipe/text 전부 end-to-end 검증.
- 🐞 **DevTools 링크 버그 수정**: 기존엔 `vmServiceUri` 를 http로 바꿔 링크했는데 그건 JSON-RPC
  엔드포인트지 DevTools가 아니었다. 데몬 `devtools.serve` 로 실제 DevTools 서버를 띄워
  `devToolsUri`(`http://host:port/?uri=<ws>`)를 노출. 9101 포트가 실제 DevTools HTML 서빙 확인.
  best-effort — DevTools 없으면 null로 링크 숨김.

### 11. iOS(및 비-Flutter) 텍스트 입력 → **조사 후 보류(2026-07-18)**
현재 `text` 는 포커스된 Flutter `EditableTextState` 에 `updateEditingValue` — Flutter 위젯엔
완벽(유니코드 포함)하나 **웹뷰 `<input>`·네이티브 뷰**엔 안 먹는다. 그 우회를 조사했다.

**후보 경로와 판정**
- ❌ **하드웨어 키보드 포워딩(`osascript` System Events keystroke)** — 실측: 이 환경에서
  `osascript`가 접근성 권한 프롬프트에서 **블록되어 강제 종료(exit 144)**. 사용자 환경에서도
  접근성 권한 부여 + Simulator 프런트모스트 + 시뮬레이터 전용이라 취약. 실기 불가.
- ⚠️ **`idb ui text`(Meta)** — 견고하나 설치 부담(brew + pip + companion 데몬)을 사용자에게 강요.
  포인터 입력 때 `idb` 를 보류한 것과 같은 이유.
- ❌ **VM Service로 Flutter `HardwareKeyboard` 키 이벤트** — Flutter 키 처리에만 닿고 네이티브/웹뷰엔
  안 닿아 문제를 못 푼다.

**결론**: 견고하면서 의존성 없는 경로가 없다. emu 핵심 대상(Flutter 위젯)은 이미 커버되고,
Flutter 디버깅에서 웹뷰/네이티브 폼 입력은 드물다. **실수요(웹뷰 폼 등)가 생기기 전엔 보류.**
수요가 생기면 `idb ui text`(설치 비용 감수)가 문서화된 선택지다. #4(e2e) 보류와 같은 성격.

### 12. 프로젝트별 계층 설정 + 학습 메모리 → **구현 완료 (2026-07-18)**
반복 검증 때 기기·플래그를 매번 재입력하지 않도록 프로젝트 설정을 **커밋 경계**로 계층화했다.

- ✅ **계층 병합**(`project_config.dart`): `emu.local.yaml`(머신 전용) > `emu.yaml`(팀 공유) >
  `~/.emu/config.yaml`(사용자 기본) > built-in. 기존 `explicit flag` > `--config` 아래에 끼움.
  순수 `mergeConfigMaps`(스칼라 덮어쓰기, 리스트 통째 교체) + `EmuConfig.fromMap` + `loadProjectConfig`.
  `up` 이 device/flavor/target/dartDefines/timeout/port/platform 전부 이 계층에서 흡수.
- ✅ **학습 메모리**(`project_memory.dart`, `.emu/memory.json`, git-ignore): 도구가 실행하며 줍는
...
- 📌 **설계 원칙**: 추적/휘발의 기준은 "머신·시간을 넘어 참인가". 기기·플래그는 참 → commit,
  좌표·화면크기는 다음 부팅에 바뀜 → 버림. 메모리를 좌표 캐시로 쓰면 e2e(#4) 보류한 브리틀함을
  뒷문으로 들이게 되므로 의미 앵커/재계산 재료만 저장한다.

### 13. Claude Code 플러그인 패키징 → **구현 완료 (2026-07-18)**
emu 리포를 플러그인 겸 셀프 마켓플레이스로 구성해 전역 설치 가능하게 했다.

- ✅ `.claude-plugin/plugin.json`·`marketplace.json`(자기 참조 `source: "./"`). `claude plugin validate` 통과.
- ✅ `skills/emu/SKILL.md` — 기동→관찰→조작→검증 루프를 Claude에게 가르침. "tap 전 항상 shot 재촬영"
  원칙을 박아 #12 메모리 학습값이 권위로 오용되지 않게 함.
- ✅ `commands/emu-setup.md` — 바이너리 빌드 + PATH 심링크 자동화.
- 📌 스킬은 CLI를 대체하지 않고 "언제·어떻게 emu를 부를지"만 심는다. 바이너리는 gitignore라
...
- **hot reload는 이미 떠 있는 `Timer.periodic`의 콜백 본문을 갈아끼우지 못할 수 있음** → 출력이 멎으면
  `restart`가 답. Flutter 시맨틱.
  ✅ **힌트 구현됨(2026-07-18)**: `reload` 직전엔 앱 로그가 활발했는데(≤5s) 직후 drain 창에 앱
  출력이 0이면 응답에 `hint`("try `emu restart`")를 넣는다. 순수 판단 `shouldHintRestart` +
  `isAppOutput`(데몬 `Reloaded N libraries` 라인 제외)로 구성, 유닛테스트.
  실측으로 버그 3개 발견·수정: (1) 데몬 reload 라인이 app 로그로 집계, (2) 활동 창 3s→5s,
  (3) **결정타 — 로그 JSON 키가 `timestamp` 가 아니라 `ts`** 라 wasActive가 항상 false였다.
