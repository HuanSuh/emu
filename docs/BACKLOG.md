...
| 5 | `emu tap`/`input` 신뢰 가능한 입력 추상화 | **높음** | **S** | **P1** | ← 다음 |
...
### 부수 확인 (2026-07-17)
- 🐞 **`emu shot [path]` 인자 무시 재확인**: 명시 경로를 줘도 조용히 버리고 `.emu/`에 저장
  (`cli.dart:661` 이 `/api/screenshot` 에 path 미전달 → `server.dart:199` 가 자체 경로 생성).
  좌표 워크플로에서 스크린샷을 반복 저장하게 되므로 #5와 함께 고치는 게 자연스럽다.
