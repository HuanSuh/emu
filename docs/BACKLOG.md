...
- ⚠️ **신규 발견 — `up` 의 `running` 은 첫 프레임 보장이 아니다**: `up` 직후 `tap` 하면 화면이 없어
  탭이 유실된다(실측). 회피: `assert --expect "<첫 화면 로그>"` 로 준비 확인 후 탭.
  → `up --wait-for-frame` 같은 옵션이나 first-frame 신호 노출을 검토할 것(P2).
- ⬜ **남은 것**: iOS 입력 수단(simctl에 tap 없음), 스와이프/텍스트 입력(`adb shell input swipe/text`).
