...
**미해결 / 다음 단계**
- 📌 **기동 버그**: emu의 `-d <udid>` 는 부팅 로직(`bootIos`)을 안 태운다 — 시뮬레이터가 미부팅이면
  flutter가 udid를 못 찾고 실패한다(`No supported devices found`). 첫 iOS 기동 실패의 원인.
  → **조치 필요(P2)**: `-d` 가 iOS 시뮬레이터 udid면 `simctl boot` 를 먼저 시도. `--android` 는
  이미 `bootAndroid` 로 running 에뮬레이터를 재사용하니, iOS `-d` 도 같은 수준으로 맞추면 된다.
